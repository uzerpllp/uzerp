 ...
 (function() {
	...
	const ClickyMenus = function( menu ) {
		...
		function menuSetup() {
			...
			menu.querySelectorAll('.clicky-menu > li > ul').forEach( ( submenu ) => {
		...
		function setUpAria( submenu, button ) {
			...
			var $num_items = submenu.querySelectorAll('ul, li').length;
			var $columns = Math.round($num_items / 10);
			if ($columns == 0) {$columns = 1};
			if ($columns > 4) {$columns = 4};
			submenu.classList.add('c-' + $columns);
		}
