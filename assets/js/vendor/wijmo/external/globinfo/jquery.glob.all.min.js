(function () {
    ...
    function l(a, e, f) {
        ...
        case "n -":
            ...
            c = " " + c;
        case "n-":
            if (o(a, b)) d = ["-", a.substr(0, a.length - b.length)];
            else if (o(a, c)) d = ["+", a.substr(0, a.length - c.length)];
            break;
        case "- n":
            b += " ";
            c += " ";
        case "-n":
            if (g(a, b)) d = ["-", a.substr(b.length)];
            else if (g(a, c)) d = ["+", a.substr(c.length)];
            break;
        case "(n)":
            if (g(a, "(") && o(a, ")")) d = ["-", a.substr(1, a.length - 2)]
        }
        return d || ["", a]
    }
    function C(h, c, m) {
        if (!c || c === "i") return m.name.length ? h.toLocaleString() : h.toString();
        c = c || "D";
        var f = m.numberFormat,
            b = Math.abs(h),
            d = -1,
            g;
        if (c.length > 1) d = parseInt(c.slice(1), 10);
        var k = c.charAt(0).toUpperCase(),
            a;
        switch (k) {
        case "D":
            g = "n";
        ...
        case "P":
            a = a || f.percent;
            g = h < 0 ? a.pattern[0] : a.pattern[1] || "n";
            if (d === -1) d = a.decimals;
            b = B(b * (k === "P" ? 100 : 1), d, a);
            break;
        default:
                ...
                function u(b, f, o) {
                    ...
                    for (; true;) {
                        ...
                        default:
                            ...
                            resource.php:
                            918Uncaught TypeError : undefined is not a
                            function