...
//echo 'system::getDirectories - '.$direction.' '.$start.' adding path '.$path.'<br>';
	...
	public function get_uzlet_resources($type)
	{
		...
		$cache = Cache::Instance();
		$files = $cache->get(array('uzlet_' . $type));
		
		$extentions = array('.uzlet.' . $type);
		...
		if ($files === FALSE)
		{
			...
			if (file_exists(PUBLIC_MODULES))
			{
				...
				$cache->add(array('uzlet_' . $type), $files);
	...
	public function get_js()
	{
		...
		$cache_id = array('resources', 'js', $this->modules['module']);
...
#	public function set_plugins()
	...
	public function setPathBase()
	{
		...
		define('SMARTY_ROOT', PLUGINS_ROOT . 'smarty' . DIRECTORY_SEPARATOR);
