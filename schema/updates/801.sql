ALTER TABLE wh_locations ADD COLUMN supply_demand boolean;

update wh_locations
   set supply_demand=has_balance;
...
CREATE OR REPLACE VIEW st_transactionsoverview AS 
 SELECT t.id, t.balance, t.created, t.latest_cost, t.std_cost, t.stitem_id, t.whlocation_id, t.whbin_id, t.transfer_id, t.whaction_id, t.process_name, t.process_id, t.qty, t.usercompanyid, t.std_mat, t.std_lab, t.std_osc, t.std_ohd, t.latest_mat, t.latest_lab, t.latest_osc, t.latest_ohd, t.remarks, t.error_qty, t.status, (l.whstore::text || '/'::text) || l.description::text AS whlocation, b.description AS whbin, (si.item_code::text || ' - '::text) || si.description::text AS stitem, (fl.whstore::text || '/'::text) || fl.description::text AS flocation, fb.description AS fbin
   FROM st_transactions t
...
CREATE OR REPLACE VIEW wh_transfers_overview AS 
 SELECT t.id, t.transfer_number, t.from_address_id, t.to_address_id, t.due_transfer_date, t.description, t.transfer_action, t.from_whlocation_id, t.to_whlocation_id, t.status, t.usercompanyid, t.actual_transfer_date, (((l1.whstore::text || '/'::text) || l1.location::text) || '-'::text) || l1.description::text AS from_location, (((l2.whstore::text || '/'::text) || l2.location::text) || '-'::text) || l2.description::text AS to_location, l1.bin_controlled AS from_bin_controlled, l2.bin_controlled AS to_bin_controlled, a.action_name
   FROM wh_transfers t
...
CREATE OR REPLACE VIEW wh_transfer_rules_overview AS 
 SELECT t.id, t.usercompanyid, t.from_whlocation_id, t.to_whlocation_id, t.whaction_id, t.created, t.createdby, t.alteredby, t.lastupdated, (((l1.whstore::text || '/'::text) || l1.location::text) || '-'::text) || l1.description::text AS from_location, (((l2.whstore::text || '/'::text) || l2.location::text) || '-'::text) || l2.description::text AS to_location, l1.bin_controlled AS from_bin_controlled, l2.bin_controlled AS to_bin_controlled, a.action_name, a.type
...
CREATE OR REPLACE VIEW st_balancesoverview AS 
 ...
 , l.whstore_id, (l.whstore::text || '/'::text) || l.description::text AS whlocation, l.supply_demand
...
CREATE OR REPLACE VIEW reports.st_balancesoverview_detail AS 
 SELECT (l.whstore::text || '/'::text) || l.description::text AS whlocation, (b.bin_code::text || ' - '::text) || b.description::text AS whbin, (si.item_code::text || ' - '::text) || si.description::text AS stitem, si.alpha_code, p.product_group, t.type_code, bl.balance, u.uom_name, round(bl.balance * si.std_cost, 2) AS valuation, round(bl.balance * si.std_mat, 2) AS matvaluation, round(bl.balance * si.std_osc, 2) AS oscvaluation, round(bl.balance * si.std_lab, 2) AS labvaluation, round(bl.balance * si.std_ohd, 2) AS ohdvaluation, round(bl.balance * si.latest_cost, 2) AS revvaluation, date_part('year'::text, now()) AS year, date_part('month'::text, now()) AS month
   FROM st_balances bl
   JOIN wh_locationsoverview l ON bl.whlocation_id = l.id
   JOIN st_items si ON bl.stitem_id = si.id
   LEFT JOIN wh_bins b ON bl.whbin_id = b.id
