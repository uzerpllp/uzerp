...
(function($) {
	...
	$.fn.collapsibleCheckboxTree = function(options) {
		...
		var defaults = {
			...
			childClickState : false, 	// Should children be shown / hidden when a parent is clicked?
		...
		this.each(function() {
			...
			/* 
			 * We don't want to auto set parent / child selections, the default values will automatically be set from the previous session
			 * 
			...
			$("input[type='checkbox']", $(this)).click(function(){
				...
				if ($(this).is(":checked")) {
					...
					if (defaults.checkParents) {
						...
						// loop through each of the parent li elements
						$(this).parents("li").each(function() {
							// We only want to tick the parent item if it is a direct child of the li element
							// NOT the first found input (might not be a direct child)
							$(this).children("input[type='checkbox']:first").attr('checked', true);
						});
