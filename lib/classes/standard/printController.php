...
class printController extends Controller {
	...
	protected $wideColumns = array('problem','description'); 
	...
	public function index(DataObjectCollection $collection, $sh='', &$c_query = null)
	{
		...
		{
			if ($fieldname != 'id'
				&& $fieldname != 'usercompanyid'
				&& substr($fieldname, -3) != '_id'
				&& !isset($model->belongsToField[$fieldname]))
			{
...
// needs extensive testing; this is implemented for selectorController output
	...
	public function select_for_output ()
	{
		...
		$this->setSearch(
			$output_details['search_do'],
			$output_details['search_method'],
			$s_data,
			array(),
			TRUE
		);
			...
			case 'process' :
				...
				if ($header)
				{
					...
					$callback = function($detail, $key) use (&$data, &$counter, &$errors, $controller, $header)
					{
						...
						$result = call_user_func(array($controller, $header->process), $detail->select_id, $data);
	...
	public function printAction()
	{
		...
		if (isset($this->_data))
		{
			...
			$ignore_keys	= array('module', 'submodule', 'controller', 'action', 'printaction', 'filename');
			$query_data		= array();
	...
	public function email_signature ()
	{
		...
		$email_signature = chr(10).chr(10).chr(10).'--'.chr(10).$company->name.chr(10).$company->getAddress()->fulladdress.chr(10).
			"Company No: ".$company->companynumber.chr(10).
			"Tel. No:    ".$company->getContactDetail('T').chr(10).
			"Fax. No:    ".$company->getContactDetail('F').chr(10);
	...
	public function printDialog()
	{
		...
		$options = call_user_func(array($this, $this->_data['printaction']), 'dialog');
...
//				'csv'	=> ''
	...
	protected function setBreakLevels($_measure_fields = array(), $_aggregate_fields = array())
	{
		...
		$measure_fields = array_merge(array('report'=>''), $_measure_fields);
	...
	public function generate_output($params, $options)
	{
		...
		else
		{
			...
			return $this->build_print_dialog_response(
				FALSE,
				array('message' => 'Error: No print type specified')
			);
			
		}
		
		// make sure we have a decent filename
		if ((!isset($params['filename']) || in_array($params['filename'], array('', 'tmp'))) && isset($options['filename']) && $options['filename']!='')
		{
		...
		if (!is_dir($paths['type_path']))
		{
			...
			if (!mkdir($paths['type_path'], 0777, TRUE))
			{
				...
				return $this->build_print_dialog_response(
					FALSE,
					array('message'=>'Error: Could not create data directory')
				);
		...
		if ($data_type === 'pdf')
		{
			...
			if (!is_dir($paths['type_path'] . 'thumbs/'))
			{
				...
				if (!mkdir($paths['type_path'] . 'thumbs/', 0777, TRUE))
				{
					...
					return $this->build_print_dialog_response(
						FALSE,
						array('message' => 'Error: Could not create PDF thumnail directory')
					);
		...
		if (!is_dir($paths['temp_path']))
		{
			...
			if (!mkdir($paths['temp_path'], 0777, TRUE))
			{
				...
				return $this->build_print_dialog_response(
					FALSE,
					array('message' => 'Error: Could not create temp directory')
				);
		 ...
		 */
		if ((!isset($options['requires_xml']) || $options['requires_xml']!==FALSE) && !in_array($data_type, array('csv', 'txt'))) {
			...
			if (empty($xml)) 
			{
				...
				return $this->build_print_dialog_response(
					FALSE,
					array('message'=>'Error: XML data is empty')
				);
		...
		switch(strtolower($data_type))
		{
			...
			case "text":
				...
				if (!isset($options['txtArray']) || count($options['txtArray']) <= 0)
				{
					...
					return $this->build_print_dialog_response(
						FALSE,
						array('message' => 'Error: No data to output as text file')
					);
			...
			case "csv":
				...
				if (isset($options['csv_source']))
				{
					...
					if (file_put_contents($csv_file, $options['csv_source']) === FALSE)
					{
						...
						return $this->build_print_dialog_response(
							FALSE,
							array('message'=>'Error: Could not create CSV document')
						);
				...
				else
				{
					...
					if (!isset($options['query']) || empty($options['query']))
					{
						...
						return $this->build_print_dialog_response(
							FALSE,
							array('message'=>'Error: No query specified')
						);
				...
				if (!chmod($paths['temp_file_path'], 0777))
				{
					...
					return $this->build_print_dialog_response(
						FALSE,
						array('message'=>'Error: Cannot update file permissions')
					);
			...
			default:
				...
				if (empty($xsl))
				{
					...
					return $this->build_print_dialog_response(
						FALSE,
						array('message'=>'Error: XSL definition is empty')
					);
				...
				if (!@copy($fop_file, $paths['temp_file_path']))
				{
					...
					return $this->build_print_dialog_response(
						FALSE,
						array('message'=>'Error: Apache FOP cannot create document')
					);
				...
				if (isset($options['merge_file_name']))
				{
					...
					if (empty($options['merge_file_name']))
					{
						...
						return $this->build_print_dialog_response(
							FALSE,
							array('message'=>'Error: Merge file not specified')
						);
					...
					else
					{
						...
						return $this->build_print_dialog_response(
							FALSE,
							array('message'=>'Error: Could not merge files')
						);
		...
		if (!file_exists($paths['temp_file_path']))
		{
			...
			return $this->build_print_dialog_response(
				FALSE,
				array('message'=>'Error: Document not generated')
			);
		...
		if (in_array($print_action, array('quick_output', 'save')))
		{
			...
			if (!@copy($paths['temp_file_path'], $paths['user_file_path']))
			{
				...
				return $this->build_print_dialog_response(
					FALSE,
					array('message'=>'Error: Could not copy file to user storage')
				);
		...
		switch ($print_action) {
			...
			case 'email':
				...
				return $this->build_print_dialog_response(
					$email,
					array('message' => "<p>Document " . $message . "successfully emailed</p>")
				);
	...
	protected function build_print_dialog_response($status, $extra = array(), $params = array())
	{
		...
		if ($status === TRUE)
		{
			...
			if (in_array($params['printaction'], array('quick_output', 'view', 'save')) && $options['pdf_browser_printing'] === TRUE && !isset($params['no_print']))
			{
				$options['buttons']['print'] = TRUE;
			}
			
			if (in_array($params['printaction'], array('quick_output', 'view', 'save')))
			{
	...
	public function get_printing_preferences($options)
	{
		...
		if ($prefs['pdf_preview'] === TRUE)
		{
			...
			$prefs['pdf_preview_build_link'] = htmlspecialchars_decode(
				link_to(
					array(
						'module'		=> $this->module,
						'controller'	=> $this->name,
	...
	public function getReportsByType($_report_type_id = '')
	{
		...
		if(isset($this->_data['ajax']) && empty($_report_type_id)) {
			$ajax = true;
			if(!empty($this->_data['report_type_id'])) { $_report_type_id=$this->_data['report_type_id']; }
		}
		else
		{
			$ajax = false;
				...
				foreach ($reverse_measure_fields as $measure_name => $blank)
				{
					...
					// START ROW
						...
						foreach ($fields as $field_slug => $fieldname)
						{
							...
							if (empty($total_level)) { $total_level=$field_formatting['normal_total']; }		
