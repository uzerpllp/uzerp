...
///	echo 'DataObjectCollection('.$this->_doname.')::_delete : expected='.$count.' actual='.$deleted.'<br>';
	...
	function setParams()
	{
		...
		if ($this->search != null && count($this->search) !== 0 && count($this->searchField) !== 0)
		{
			...
			{
				$this->searchString .= ' LOWER(' . $this->searchField[0] . ') LIKE ' . "'" . strtolower((string) $s) . "%' AND";
				$pointer++;
			}

 			$this->searchString = substr((string) $this->searchString, 0, (strlen((string) $this->searchString) - 3));
