...
abstract class Controller
{
    ...
    public function setSearchHandler(DataObjectCollection $collection, $search_id = NULL, $force_use_session = false)
    {
        ...
        if ($force_use_session || (isset($this->search) && isset($this->_data['ajax_print'])) || isset($this->_data['orderby']) || isset($this->_data['page'])) {
    ...
    public function saveAnother()
    {
        ...
        sendTo($_GET['controller'], $this->_data['original_action'], array(
            $_GET['module']
        ), $res['other']);
    }
    ...
    protected function setSearch($do, $method, $defaults = array(), $params = array(), $use_saved_search = false)
    {
        ...
        } elseif (! isset($this->_data['orderby']) && ! isset($this->_data['page']) && ! $use_saved_search) {
            $s_data = $defaults;
        }
     ...
     *            only show actions with these names in the related sidebar
     */
    ...
    public function sharing($model = '')
    {
        ...
        if ($object->owner != EGS_USERNAME && ! isModuleAdmin()) {
            ...
            if ($objectPermissions->getRows($object->id, $object->getTableName(), 'write')
                ->count() == 0) {
                if (empty($model)) {
                    $flash = Flash::Instance();
                    $flash->addError('You do not have permission to edit this ' . $modelname);
                    sendTo($this->name, 'view', $this->_data['module'], array(
                        'id' => $this->_data['id']
                    ));
                }
                return false;
