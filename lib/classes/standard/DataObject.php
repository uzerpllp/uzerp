...
//				$string.=$this->$fieldname.' ';
	 ...
	 * @param string $field the name of the foreign-key field in the do (defaults to the do-class-name suffixed with 'id')
	 * @param string $name the name by which the object can later be referenced
	 * @param string $field the name of the parent field in the current object (defaults to the current object id field)
	 * @return boolean
	 */
	 ...
	 * @param string $name the name by which the object can later be referenced
	 * @param string $cc a constraint chain declaration to restrict drop down list
	 * @param string $identifierField to be used to populate the drop down list
	 * @return boolean
	 */
...
//			$this->getField($field)->addValidator(new ForeignKeyValidator($do));
	 ...
	 * when the object is
	 * @param	string	the name of the DO that is to be registered
	 * @param	string	the name that should be given to the collection (defaults to $name.'s')
	 * @param	string	the name of the foreign-key field in the dependent (defaults to the current-class-name suffixed with 'id')
	 * @param	string	the name of the parent field in the foreign-key relationship (defaults to the parent id field)
	 * @param	boolean	specifies whether to allow cascade delete (defaults to true)
	 * @return boolean
	 */
	 ...
	 *
	 * @param	string	$modelName	the name of the model to inherit
			...
			if (!empty($rows))
			{
				foreach ($rows as $value)
				{
					...
					if (strtolower(get_class($this))==$module_component)
					{
						...
						$this->_policyConstraint['constraint']->add(new Constraint($value['fieldname'], $value['operator'], $value['value'])
									, $value['type']
...
//	echo 'DataObject('.get_class($this).')::getChildren '.$query.'<br>';
	 ...
	 * @param	&$options	array	An array passed-by-reference with a list of options
	 */
	 ...
	 *
	 * @param	$field		string	The field for which to fetch options
	 * @param	$field		string	The options value to search for
...
//		echo 'DataObject::getAccessConstraint type='.$accessType.' '.$this->_accessContraint[$accessType]->__toString().'<br>';
	...
	public function getLinkRules()
	{
		...
		foreach ($this->getHasMany() as $name => $detail)
		{
			...
			{
				$hasMany[$name] = $detail;
				$hasMany[$name]['actions'] = array('link', 'new');
			}
			else
			{
