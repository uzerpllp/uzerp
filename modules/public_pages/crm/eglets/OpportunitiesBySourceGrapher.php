...
class OpportunitiesBySourceGrapher extends SimpleGraphEGlet {
	...
	function populate() {
		...
		$query = $db->prepare("select s.name, COALESCE(sum(o.value), 0.0) AS piplinevalue FROM opportunitysource s LEFT OUTER JOIN opportunities o ON (s.id=o.source_id
		AND o.usercompanyid=? AND extract('month' FROM o.enddate)=extract('month' FROM now())
		AND extract('year' FROM o.enddate)=extract('year' FROM now())) WHERE s.usercompanyid=? GROUP BY s.name ORDER BY COALESCE(sum(o.value), 0.0), s.name");
