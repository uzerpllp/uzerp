...
class HolidayrequestsController extends HrController
{
	...
	public function _new()
	{
		...
		if ($holidayRequest->isLoaded() && $holidayRequest->special_circumstances=='f')
		{
			...
			$this->view->set('days_left', bcadd((string) $days_left, $holidayRequest->num_days, 1));
		}
		else
		{
			// New request, or editing existing request for Special Circumstances
			$this->view->set('days_left', $days_left);
			$this->view->set('new_days_left', bcsub((string) $days_left, $holidayRequest->num_days, 1));
		}
...
//		parent::delete('Holidayrequest');
	...
	public function save($modelName = null, $dataIn = [], &$errors = []) : void
	{
		...
		if (isset($this->_data[$this->modeltype]['start_date'])
			...
			&& count($errors) == 0
			&& strtotime((string) fix_date($this->_data[$this->modeltype]['start_date'])) > strtotime((string) fix_date($this->_data['Holidayrequest']['end_date'])))
		{
...
// Holiday can only be authorised by an 'authoriser'
	...
	public function getHolidays()
	{
		...
		foreach($holidayrequests as $key=>$value)
		{
			...
			$output_events[]=array('id'=>$value['id'],
							...
							'start'=>strtotime((string) $value['start_date']),	
	...
	public function getNumberDays($_employee_id = '', $_start_date = '', $_end_date = '', $_all_day = true)
	{
		...
		$date = strtotime((string) $_start_date);

		for ($date = strtotime((string) $_start_date), $days = 0; $date <= strtotime((string) $_end_date); $date = strtotime(' +1 day', $date))
		{
			if (date("N",$date) < 6)
			{
				$days++;
