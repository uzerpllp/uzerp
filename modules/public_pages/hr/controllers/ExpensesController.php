...
//		$saved_data = $this->decode_original_form_data($this->_data['encoded_query_data']);
		...
		$employee_address = array('employee'=>$employee->person->titlename);
		...
		$expense_totals[]['line'] = array('label' => 'NET VALUE', 'value' => number_format($net_total, 2, '.', '').' '.$expense->currency);
		$expense_totals[]['line'] = array('label' => 'VAT', 'value' => number_format($vat_total, 2, '.', '').' '.$expense->currency);
		$expense_totals[]['line'] = array('label' => 'EXPENSE TOTAL','value'=>number_format($expense_total, 2, '.', '').' '.$expense->currency);
	...
	{
		if (!$this->loadData()
		&& !isset($this->_data['expense_number']))
		{
			$this->dataError();
			sendBack();
		}
		...
		$sidebarlist[$expense->employee_id] = array('tag' => $expense->employee
													,'link' => array('modules'		=> $this->_modules
																	,'controller'	=> $this->name
																	,'action'		=> 'index'
		...
		$sidebarlist['print_expense_claim'] = array('tag'=>'print expense claim'
													,'link'=>array('modules'		=> $this->_modules
																  ,'controller'		=> $this->name
																  ,'action'			=> 'printDialog'
		...
		if ($expense->awaitingAuthorisation()
			|| ($expense->authorised()
				&& $expense->employee_id!=$this->get_employee_id()) )
		{
			$sidebarlist['cancel']=array('tag' => 'Cancel Expenses'
										,'link'=> array('modules'		=> $this->_modules
													   ,'controller'	=> $this->name
													   ,'action'		=> 'delete'
													   ,$idField		=> $idValue
		...
		if ($expense->awaitingAuthorisation())
		{
			$sidebarlist['edit'] = array('tag' => 'Amend Expenses'
										,'link' => array('modules'		=> $this->_modules
														,'controller'	=> $this->name
														,'action'		=> 'edit'
														,$idField		=> $idValue
			...
			$sidebarlist['add_lines']=array('tag'=>'Add_Lines'
										   ,'link'=>array('modules'				=> $this->_modules
		...
		{
			$sidebarlist['post'] = array('tag' => 'Post to Ledger'
										,'link' => array('modules'		=> $this->_modules
														,'controller'	=> $this->name
														,'action'		=> 'post'
														,$idField		=> $idValue
		...
		{
			$sidebarlist['post'] = array('tag' => 'Pay this Expense Claim'
										,'link' => array('modules'		=> $this->_modules
														,'controller'	=> $this->name
														,'action'		=> 'make_payment'
														,$idField		=> $idValue
...
// Expenses can only be authorised by an 'authoriser'
		if ($employee->expense_model()->isAuthorised($employee->id, $this->get_employee_id())
				&& $expense->awaitingAuthorisation()
				&& $expense->gross_value != 0)
		{
			$sidebarlist['confirm'] = array('tag' => 'Authorise Expenses'
										,'link'=> array('modules'		=> $this->_modules
													   ,'controller'	=> $this->name
