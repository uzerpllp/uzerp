...
class GLTransaction extends DataObject
{
	...
	protected $defaultDisplayFields = array('transaction_date'	=> 'Date'
											,'account'			=> 'Account'
...
// Define enumerated types
		$this->setEnum('type', array('A'	=> 'Accural'
									,'C'	=> 'Credit Note'
		...
		$this->setEnum('source', array('A' => 'Asset Register'
									  ,'C' => 'Cash Book'
									  ,'E' => 'Expenses'
									  ,'G' => 'General Ledger'
									  ,'P' => 'Purchase Ledger'
	...
	public function makeFromAsset (Asset $asset, $value, $type, &$errors=array())
	{
		...
		if (empty($gl_data['glaccount_id'])
			|| empty($gl_data['glcentre_id'])
			|| empty($gl_data['credit_glaccount_id'])
			|| empty($gl_data['credit_glcentre_id'])
			)
		{
