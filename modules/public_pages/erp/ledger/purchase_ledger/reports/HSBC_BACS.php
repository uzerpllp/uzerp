...
class HSBC_BACS
{
	...
	function constructPrint($data, $options)
	{
		...
		$response = json_decode((string) $this->controller->constructOutput($data, $options));
	...
	function validate($data, &$errors = [])
	{
		...
		if (in_array(date('D', strtotime((string) $payment_date)), array('Sat', 'Sun')))
		{
	...
	function setData($bacs_reference, $payments, &$errors = array(), $data = [], PLPayment $plpayment = null)
	{
		...
		$processing_date = bcadd(date('z', strtotime((string) $this->getProcessDate($plpayment->payment_date))), 1, 0);
	...
	private function getProcessDate($payment_date)
	{
		...
		$payment_date	= strtotime((string) $payment_date);
	...
	private function formatReport()
	{
		...
		$ssn		= '99' . str_pad((string) $db->GenID('bacs_file_id_seq'), 4, '0', STR_PAD_LEFT);
		...
		foreach ($this->payments as $payment)
		{
			...
			$lines[] = $line['sort_code']
				...
				.$line['negate']
				.$line['payer_name']
				.str_pad((string) $line['our_reference'], 18, ' ', STR_PAD_RIGHT)
				.str_pad(substr((string) $line['payee_name_check'], 0, 18), 18, ' ', STR_PAD_RIGHT)
				.$line['pay_date'];
