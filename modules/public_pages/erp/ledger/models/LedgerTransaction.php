...
// Define enumerated types
	...
	public static function setCurrency (&$data)
	{
		...
		{
			$data['base_gross_value']	= bcadd(round(bcdiv((string) $data['gross_value'],(string) $data['rate'],4),2),0);
			$data['base_tax_value']		= bcadd(round(bcdiv((string) $data['tax_value'],(string) $data['rate'],4),2),0);
			$data['base_net_value']		= bcadd(round(bcsub((string) $data['base_gross_value'],$data['base_tax_value']),2),0);
		}
		...
		{
			$data['twin_gross_value']	= bcadd(round(bcmul((string) $data['base_gross_value'],(string) $data['twin_rate'],4),2),0);
			$data['twin_tax_value']		= bcadd(round(bcmul((string) $data['base_tax_value'],(string) $data['twin_rate'],4),2),0);
			$data['twin_net_value']		= bcadd(round(bcsub((string) $data['twin_gross_value'],$data['twin_tax_value']),2),0);
		}
	...
	public function setTwinBaseValue ($fieldname)
	{
		...
		else
		{
			...
			$this->$basefield = bcadd(round(bcdiv((string) $this->$fieldname,$this->rate,4),2),0);
		}
		...
		{
			// use bcadd to format result
			$this->$twinfield = bcadd(round(bcmul((string) $this->$basefield,$this->twin_rate,4),2),0);
		}
