...
// used by select_for_output function
		$this->output_types=array('statement'=>array('search_do'=>'SLCustomerSearch'
													,'search_method'=>'statements'
													,'search_defaults'=>array('statement'=>'TRUE')
													,'collection'=>'SLCustomerCollection'
													,'collection_fields'=>array('id','name','currency','outstanding_balance','email_statement as email','last_statement_date')
													,'display_fields'=>array('name','outstanding_balance','currency','last_statement_date','email')
...
// Set context from calling module
	...
	{
		return $this->getOptions($this->_uses['SLTransaction'], 'slmaster_id', 'getCustomerList', 'getOptions', array('use_collection'=>true));
	...
	public function save_allocation()
	{
		...
		{
			if (!SLTransaction::allocatePayment($transactions, $this->_data['id'], $errors)
	...
	public function outstanding_transactions()
	{
		...
		$can_contra = (isset($categories['PL']['exists'])
						&& $categories['PL']['exists']
	...
	public function save_contras()
	{
		...
		if (count($errors)>0
			|| !SLTransaction::allocatePayment($transactions, $this->_data['id'], $errors)
			|| !SLAllocation::saveAllocation($transactions, $errors))
		{
			$db->FailTrans();
		}
	...
	public function printStatement($status='generate')
	{
		...
		*/
		...
		if (strtolower($this->_data['printaction'])=='printstatement'
			&& !is_null($customer->email_statement()))
		{
	...
	{
		if (!$this->checkParams(array('id', 'account_status')))
		{
			sendBack();	
		}
	...
	public function getAccountRate($_customer_id = '', $_cb_account_id = '')
	{
		...
		if(isset($this->_data['ajax']))
		{
			if(!empty($this->_data['id'])) { $_customer_id=$this->_data['id']; }
		...
		if ($customer->isLoaded()
			&& $glparams->base_currency() != $customer->currency_id)
		{
	...
	public function getCustomerData()
	{
		...
		$output['person_id']=array('data'=>$people,'is_array'=>is_array($people));
	...
	public function getPeople($_company_id = '')
	{
		...
		if(empty($_company_id) && isset($this->_data['company_id'])) { $_company_id=$this->_data['company_id']; }
	...
	private function getCustomer($_customer_id = '')
	{
		...
		if(isset($this->_data['ajax']))
		{
			if(!empty($this->_data['slmaster_id'])) { $_customer_id = $this->_data['slmaster_id']; }
		}
	...
	private function sidebar($current_type)
	{
		...
		$sidebarlist['all'] = array('tag'=>'View all customers',
									'link'=>array('modules'=>$this->_modules
												 ,'controller'=>$this->name
												 ,'action'=>'index'
								 				 )
		...
		{
			$sidebarlist['receive_payment'] = array('tag' => 'receive_payment',
													'link'=>array('modules'=>$this->_modules
																 ,'controller'=>$this->name
																 ,'action'=>'receive_payment'
																 )
		...
		{
			$sidebarlist['make_refund'] = array('tag' => 'make_refund',
													'link'=>array('modules'=>$this->_modules
																 ,'controller'=>$this->name
																 ,'action'=>'make_refund'
																 )
		...
		{
			$sidebarlist['enter_journal'] = array('tag'=>'Enter SL Journal',
												  'link'=>array('modules'=>$this->_modules
															   ,'controller'=>$this->name
															   ,'action'=>'enter_journal'
															   )
