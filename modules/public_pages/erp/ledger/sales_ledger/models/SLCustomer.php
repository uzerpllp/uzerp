...
// Define relationships
		...
		$this->setComposite('Address', 'billing_address_id', 'billing_address', array('street1', 'street2', 'street3', 'town', 'county', 'postcode', 'countrycode'));
		
		$cc = new ConstraintChain();
 		$cc->add(new Constraint('invoice_method','==','"E"'));
		$this->addValidator(new DependencyValidator($cc,array('email_invoice_id','email_statement_id')));
	...
	public function getOutstandingOrders()
	{
		...
		foreach($this->orders as $order)
		{
			...
			else
			{
				...
				{
					if(in_array($lines->status, array('N','R','D')))
					{
