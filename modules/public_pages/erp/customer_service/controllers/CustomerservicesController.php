...
class CustomerServicesController extends PrintController {
	...
	public function detail()
	{
		...
		if (!isset($this->_data['orderby']) && !isset($this->_data['page'])
				&& !($this->isPrintDialog() || $this->isPrinting()))
		{
...
//			echo '$s_data<pre>'.print_r($s_data, true).'</pre><br>';
		...
		$sh->extractOrdering();
		$sh->setFields(array('id','product_group','customer','stitem','order_number','despatch_number','due_despatch_date','despatch_date','order_qty','despatch_qty','failurecode','cs_failurecode_id'));
	...
	public function savefailure ()
	{
		...
		if ($cs_failure->update($this->_data[$this->modeltype]['id'], 'cs_failurecode_id', $this->_data[$this->modeltype]['cs_failurecode_id']))
		{
	...
	public function printCustomerService()
	{
		...
		if ($this->_data['print']['printtype'] === 'csv')
		{
			...
			$options['csv_source'] = $this->generate_csv($this->_data['print'], $csv_data, array('customer'
																								,'ontime'
																								,'infull'
																								,'ontime_infull'
																								,'count'
	...
	protected function setSearch ()
	{
		...
		elseif (!isset($this->_data['orderby']) && !isset($this->_data['page'])
				&& !(isset($this->search) && isset($this->_data['ajax_print']))
				&& !($this->isPrintDialog() || $this->isPrinting()))
		{
