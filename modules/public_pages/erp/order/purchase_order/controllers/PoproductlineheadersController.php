...
// Get any Purchase Orders for the item, by date
			...
			foreach ($porders as $porder) {
				...
				{
					$orders[$porder->due_delivery_date.'PO'.$porder->order_id]=array('due_date'=>un_fix_date($porder->due_delivery_date),
								'order_type'=>'PO',
...
// Get any Works Orders to make this item, by date
			...
			foreach ($worders as $worder) {
				$orders[$worder->required_by.'WO'.$worder->id]=array('due_date'=>un_fix_date($worder->required_by),
								'order_type'=>'WO',
...
// Get any Sales Orders by date for the item
			...
			foreach ($sorders as $sorder) {
				...
				{
					$orders[$sorder->due_despatch_date.'SO'.$sorder->order_id]=array('due_date'=>un_fix_date($sorder->due_despatch_date),
								'order_type'=>'SO',
...
// Get any Works Orders that use this item, by date
			...
			foreach ($wostructures as $wostructure) {
				$orders[$wostructure->required_by.'WO'.$wostructure->work_order_id]=array('due_date'=>un_fix_date($wostructure->required_by),
								'order_type'=>'WO',
...
// Get any Purchase Orders for items that use this item, by date
			...
			foreach ($porders as $porder) {
				...
				{
					$orders[$porder->due_delivery_date.'PO'.$porder->id]=array('due_date'=>un_fix_date($porder->due_delivery_date),
								'order_type'=>'PO',
								'reference'=>$porder->order_number,
								'reference_id'=>$porder->order_id,
								'stitem_id'=>$porder->stitem_id,
 ...
 */
	...
	protected function setSidebarView($po_pl_header)
	{
		...
		$this->sidebarActions($sidebar, $po_pl_header, array('delete'=>false));
