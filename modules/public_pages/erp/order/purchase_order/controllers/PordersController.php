...
// Set context from calling module
		...
		$s_data['raised_by']=array('raised_by'=>$user->username
...
// get supplier list
		...
		{
			$suppliers = array($porder->plmaster_id=>$porder->supplier);
		}
		else
		{
...
// get the default/current selected supplier
	...
	public function view()
	{
		...
		if (($order->awaitingDelivery()
			&& $order->allLinesAwaitingDelivery($linestatus))
			|| ($linestatuses['linecount'] == 0 && !$order->isNew()))
		{
	...
	public function profile()
	{
		...
		$sh->setFields(array('id', 'order_number', 'glaccount_id', 'glcentre_id', 'value'));
...
// Set context from calling module
		...
		$poreceivedline->setDefaultDisplayFields(array('gr_number'
													  ,'order_number'
													  ,'delivery_note'
													  ,'supplier'
													  ,'received_date'
...
// Used by Ajax to return Product Lines list after selecting the Customer
	...
	{
		if($_plmaster_id=='') { $_plmaster_id = $this->_data['plmaster_id']; }
...
// Used by Ajax to return Notes after selecting the Supplier
		if($_supplier_id=='' && !empty($this->_data['id'])) { $_supplier_id = $this->_data['id']; }
	...
	public function resetstatus ()
	{
		...
		else
		{
			...
			if (count($errors)>0
				|| !$order->update($this->_data['id'], 'status', $order->newStatus()))
			{
	...
	public function cancel_order ()
	{
		...
		else
		{
			foreach ($order->lines as $line)
			{
				...
				{
					if (!$line->update($line->id,
									   array('status', 'glaccount_centre_id'),
									   array($line->cancelStatus(), 'null')))
					{
						$errors[] = $db->ErrorMsg();
...
// Set context from calling module
	...
	public function saveinvoice()
	{
			...
			if ($rows)
			{
				...
				foreach ($rows as $received_line)
				{
					...
					if ($porderline->isLoaded())
					{
						...
						if ($porderline->del_qty == $invoiced_qty
						 && $porderline->os_qty	 == 0)
						{
		...
		if ($result)
		{
			...
			if (!$poreceivedlines->update(array('invoice_number', 'invoice_id'), array($pinvoice->invoice_number, $pinvoice->id), $sh))
			{
		...
		else
		{
			...
			sendTo('pinvoices', 'view', 'purchase_invoicing', array('id'=>$pinvoice->id));
		}
...
// Set context from calling module
	...
	public function printOrder($status = 'generate', $report = 'PurchaseOrder')
	{
		...
		$document_reference[]['line']	= array('label' => 'Order Number', 'value' => $order->order_number);
		$document_reference[]['line']	= array('label' => 'Our Reference', 'value' => $order->our_reference);
		...
		$supplier_address =array('title' => 'To:', 'name' => $supplier->name);
		...
		// set billing address
		$billing_address = array('title' => 'Billing Address:', 'name' => $extra['company_name']);
	...
	public function printOrderList($status = 'generate')
	{
		...
		switch ($this->_data['type'])
		{
			...
			case ('overdue'):
				...
				$sh->setOrderby(array('supplier', 'due_date'), array('ASC', 'ASC'));
				break;
			case ('outstanding'):
				$sh->addConstraint(new Constraint('status', ' NOT IN', "('I', 'R', 'X')"));
				$sh->setOrderby('order_number');
		...
		$extra = array('title'=>prettify($this->_data['type']).' Purchase Orders as at '.un_fix_date(fix_date(date(DATE_FORMAT))));
		...
		// generate the xml and add it to the options array
		$options['xmlSource'] = $this->generateXML(array('model'=>$porders,
														 'extra'=>$extra,
	...
	public function getSupplierData() {
		...
		$output['receive_action']	= array('data'=>$receive_action, 'is_array'=>is_array($receive_action));
		...
		$output['delivery_term'] = array('data'=>$supplier_term, 'is_array'=>is_array($supplier_term));
