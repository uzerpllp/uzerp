	...
	{data_table}
		...
		{foreach name=datagrid item=model from=$poproductlines}
			{grid_row model=$model}
				...
				{/grid_cell}
				{grid_cell model=$model cell_num=4 field="glaccount"}
					{$model->glaccount}
				{/grid_cell}
				{grid_cell model=$model cell_num=5 field="glcentre"}
					{$model->glcentre}
				{/grid_cell}
				{grid_cell model=$model cell_num=6 field="stitem"}
					{$model->stitem}
				{/grid_cell}
				...
				{grid_cell model=$model cell_num=8 field="currency"}
					{$model->currency}
				{/grid_cell}
				{grid_cell model=$model cell_num=9 field="price"}
					{$model->getPrice()|string_format:"%.4f"}
				{/grid_cell}
			{/grid_row}
		{foreachelse}
			<tr>
