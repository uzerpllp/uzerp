...
{content_wrapper}
	<div id="view_page" class="clearfix">
		...
		{form controller="sorders" action="save_cancel_lines" notags=true}
				...
				{foreach name=lines item=line from=$sorder->lines}
					{if $line->status==$line->newStatus()}
						<tr>
							...
							<td><input type='checkbox' name=SOrderLine[cancel_line][{$line->id}] id=SOrderLine_cancel_line_{$line->id} value=true></td>
						</tr>
					{/if}
				{/foreach}
			</table>
