...
class SalesOrdersSummaryEGlet extends SimpleEGlet
{
	...
	function populate()
	{
		...
		$weekend	= fix_date(date(DATE_FORMAT, strtotime("next Monday", strtotime((string) $today))-1));
		$startdate	= fix_date('01/'.date('m/Y'));
		$monthend	= fix_date(date(DATE_FORMAT, strtotime("+1 months", strtotime((string) $startdate))-1));
		$nextmonth	= fix_date(date(DATE_FORMAT, strtotime("+2 months", strtotime((string) $startdate))-1));
