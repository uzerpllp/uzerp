...
// so set context from calling module
	...
	public function clone_order()
	{
		...
		if ($result!==FALSE)
		{
			sendTo($this->name, 'view', $this->_modules, array($order->idField=>$result['internal_id']));
		}
...
// get people based on customer
	...
	public function save()
	{
		...
		if (!empty($header['ext_reference']))
		{
			...
			$check_sorder->loadBy(array('slmaster_id', 'ext_reference')
								, array($header['slmaster_id'], $header['ext_reference']));
	...
	public function view()
	{
		...
		$actions['allcustomer']=array(
					'link'=>array('module'=>'sales_ledger'
		...
		if ($order->type=='O')
		{
			...
			if ($order->someLinesDespatched($linestatus)
				|| $order->someLinesPicked($linestatus))
			{
...
// Used by Ajax to return Default Despatch Action after selecting the Supplier
	...
	public function getPersonAddresses($_person_id='',$_type='',$_slmaster_id='') {
		...
		if(isset($this->_data['ajax'])) {
			...
			if(!empty($this->_data['type'])) { $_type=$this->_data['type']; }
			if(!empty($this->_data['slmaster_id'])) { $_slmaster_id=$this->_data['slmaster_id']; }
		}
		...
		if (isset($sorderline_data)) {
			foreach ($sorderline_data as $key=>$value) {
				...
				if (!is_null($sorderline->stitem_id)) {
					if (!empty($value['whlocation_id'])
						&& !empty($sorder_data['to_location_id'])) {
						$data=array();
...
//					$sorderline->os_qty+=$value['del_qty'];
				...
				if (!is_null($sorderline->stitem_id)) {
					if (!empty($value['whlocation_id'])
						&& !empty($sorder_data['from_location_id'])) {
						$data=array();
						$data['stitem_id']=$sorderline->stitem_id;
						$data['qty']=$value['del_qty'];
						$data['process_name']='SO';
	...
	public function confirm_sale ()
	{
		...
		{
			$this->view->set('inv_addresses', $sorder->getPersonAddresses($sorder->person_id, array('type'=>'billing')));
			$this->_data['context'] = 'confirm_sale';
			$this->view->set('del_addresses', $sorder->getPersonAddresses($sorder->person_id, array('type'=>'shipping')));
			$this->view->set('people',array($sorder->person_id=>$sorder->person));
		}
		else
		{
			$this->view->set('inv_addresses', $sorder->getInvoiceAddresses());
			$this->view->set('del_addresses', array(''=>'Same as Invoice Address')+$sorder->getDeliveryAddresses());
...
// Match Payment to Invoice
		if (count($errors)==0)
		{
			if ($invoice->isLoaded())
			{
				...
				if (!SLTransaction::allocatePayment($transactions, $invoice->slmaster_id, $errors)
				||  !SLAllocation::saveAllocation($transactions, $errors))
				{
...
// now print the invoice to the users default printer
	...
	public function getDeliveryTerm ($_slmaster_id='') {
		if($_slmaster_id=='' && !empty($this->_data['slmaster_id'])) { $_slmaster_id=$this->_data['slmaster_id']; }
	...
	private function createPostInvoice($sorder, &$errors)
	{
		...
		if (count($errors) == 0
		&& count($create_sodespatchlines)>0
		&& !SODespatchLine::createDespatchNote($create_sodespatchlines, $errors))
		{
		...
		if (count($errors) == 0)
		{
			...
			foreach ($sodespatchlines as $id=>$data)
			{
				...
				if ($sodespatchlines->update(
						array('invoice_id', 'invoice_number'),
						array($sinvoice->id, $sinvoice->invoice_number),
						$sh
					)===false)
				{
	...
	protected function printInvoice($invoice,$data)
	{
		...
		$options=array('filename'	=> 'SalesInvoice'.$invoice->id,
					   'report'		=> 'SalesInvoice'
					  );
		...
		$options['xmlSource']=$this->generateXML(array('model'=>$invoice,
	...
	public function printOrderList($status='generate') {
		...
		// build extra array
		$extra=array('title'=>prettify($title).' Sales Orders as at '.un_fix_date(fix_date(date(DATE_FORMAT))),
					 'showlines'=>true
					);
		
		// generate the xml and add it to the options array
		$options['xmlSource']=$this->generateXML(array('model'=>$orders,
													   'extra'=>$extra,
													   'relationship_whitelist'=>array('lines')
													  )
												);
...
//			$extra['lines'][] = $orderlines;
	...
	public function printProFormaInvoice($status = 'generate')
	{
		...
		{
			echo $this->returnResponse(FALSE, array('message'=>'No lines selected for the Pro Forma'));
			exit;
		}
		...
		if (!is_null($order->person_id))
		{
			$inv_address = array('person'=>$order->person);
		}
...
//			$tax_total=trunc(bcmul($orderlines->net_value,$tax_percentage,4),2);
		...
		$invoice_totals[]['line'] = array('label' => 'NET VALUE', 'value' => number_format($net_total, 2, '.', '').' '.$order->currency);
		$invoice_totals[]['line'] = array('label' => 'VAT', 'value' => number_format($vat_total, 2, '.', '').' '.$order->currency);
		$invoice_totals[]['line'] = array('label' => strtoupper($order->getFormatted('type')).  ' TOTAL','value'=>number_format($inv_total, 2, '.', '').' '.$order->currency);
	...
	public function printPickList($status='generate')
	{
		...
		$line_status = array('\''.$line->newStatus().'\'','\''.$line->partDespatchStatus().'\'');
		...
		$order_details[]['line'] = array('label'=>'Picked By:','value'=>$user->getPersonName());
		$order_details[]['line'] = array('label'=>'Order Date:','value'=>un_fix_date($order->order_date));
		$order_details[]['line'] = array('label'=>'Our Order Number:','value'=>$order->order_number);
		$order_details[]['line'] = array('label'=>'Customer Ref:','value'=>$order->ext_reference);
		$order_details[]['line'] = array('label'=>'Due Date:','value'=>un_fix_date($order->due_date));
		...
		// generate the xml and add it to the options array
		$options['xmlSource'] = $this->generateXML(array('model'=>array($order),
													     'extra'=>$extra,
													     'relationship_whitelist'=>array('lines')
													     )
												  );
	...
	public function getBalance ($_stitem_id='', $_location_id='')
	{
		...
		if(isset($this->_data['ajax']))
		{
			...
			if(!empty($this->_data['location_id'])) { $_location_id=$this->_data['location_id']; }
		}
	...
	public function getCustomerData()
	{
		...
		$output['payment_terms'] = array('data'=>$payment_terms,'is_array'=>is_array($payment_terms));
	...
	public function getPersonData()
	{
		...
		{
			$del_address_id = array(''=>'Same as Invoice Address')+$del_address_id;
		}
		$output['del_address_id'] = array('data'=>$del_address_id,'is_array'=>is_array($del_address_id));
	
		$inv_address_id = $this->getPersonAddresses($_person_id,$_inv_type,$_slmaster_id);
		$output['inv_address_id'] = array('data'=>$inv_address_id,'is_array'=>is_array($inv_address_id));
						
		$notes = $this->getNotes($_person_id,$_slmaster_id);
		$output['notes'] = array('data'=>$notes,'is_array'=>is_array($notes));
