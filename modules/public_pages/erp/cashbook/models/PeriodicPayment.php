...
//		$this->orderby='';
		...
		$this->adder = array(
			'W'	=> array('type' => 'days', 'multiplier' => 7),
			'M'	=> array('type' => 'months', 'multiplier' => 1),
			'Q'	=> array('type' => 'months', 'multiplier' => 3),
			'Y'	=> array('type' => 'months', 'multiplier' => 12)
		);
	...
	public static function Factory($data, &$errors = array(), $do_name = null)
	{
		...
		if (!empty($data['source'])
			&& ($data['source'] == 'CR'
				|| $data['source'] == 'CP'))
		{
			$model->addValidator(new fkFieldCombinationValidator('GLAccountCentre', array('glaccount_id'=>'glaccount_id','glcentre_id'=>'glcentre_id')));
		}
