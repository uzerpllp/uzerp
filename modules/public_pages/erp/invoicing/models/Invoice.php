...
//		so initialise the values to zero
	...
	public function post(&$errors=array())
	{
		...
		foreach ($this->lines as $line)
		{
			...
			foreach ($fields as $field)
			{
				$linesum[$field]=bcadd($linesum[$field], (string) $line->$field);
			}
		}
...
//			$gl_transactions = GLTransaction::makeFromLedgerTransaction($transaction, $this, $errors);
	...
	/**
	 * Return a new Ledger Transaction
	 *
	 * @return \DataObject  Ledger Transaction
	 */
	abstract protected function transactionFactory(): \DataObject;
	
	/**
	 * Get the control account for a Ledger
	 *
	 * @param \DataObject $gl_params  GLParams object
	 * @param array $errors
	 * @return \int  GL Account ID
	 */
	abstract protected function get_ledger_control_account(\DataObject $gl_params, &$errors = []): int;
	...
	private function makeLedgerTransaction()
	{
		...
		if ($transaction->transaction_type == 'C')
		{
			...
			{
				if(substr((string) $fieldname,-5,5) == 'value') {
					$transaction->$fieldname = bcmul((string) $transaction->$fieldname, -1);
				}
			}
		}
