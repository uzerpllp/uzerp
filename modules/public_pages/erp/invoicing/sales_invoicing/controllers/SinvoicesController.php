...
// used by select_for_output function
		$this->output_types=array('invoice'=>array('search_do'=>'sinvoicesSearch'
													,'search_method'=>'invoices'
													,'search_defaults'=>array()
													,'collection'=>'SInvoiceCollection'
													,'collection_fields'=>array('id','invoice_number','invoice_date','customer','currency','gross_value','invoice_method as method','email_invoice as email')
													,'display_fields'=>array('invoice_number','invoice_date','customer','gross_value','currency','email')
...
// set context from calling module
		...
		if (isset($this->_data['from']) && isset($this->_data['to'])) {
			$s_data['invoice_date']=array('from'=>un_fix_date($this->_data['from']), 'to'=>un_fix_date($this->_data['to']));
		}
...
// Used by Ajax to return Notes after selecting the Customer
	...
	public function getPersonAddresses($_person_id='',$_type='',$_slmaster_id='') {
		...
		$_data=Array('type'=>$_type,'slmaster_id'=>$_slmaster_id);
...
// Set context from calling module
	...
	public function printInvoice($status='generate')
	{
		...
		if ($this->_data['print']['printaction']=='EDI' && !is_null($invoice->customerdetail->edi_invoice_definition_id))
		{
			...
			$edi->processFile(array('filename'=>$filename ,'data'=>$invoice->id), $errors);
			
			if(count($errors)>0)
			{
				echo json_encode(array('status'=>false,'message'=>'EDI failed'));
			}
			else
			{
				echo json_encode(array('status'=>true,'message'=>'EDI success'));
			}
		...
		else
		{
			...
			$xml=$this->generateXML(array('model'=>$invoice,'relationship_whitelist'=>array('lines'),'extra'=>$extra));
			...
			{
				$invoice->update($this->_data['id'], array('date_printed', 'print_count'), array(fix_date(date(DATE_FORMAT)), $invoice->print_count+1));
			}
