...
class assetHandling
{
	...
	static function depreciateAsset($asset, &$errors = [])
	{
		...
		if ($depn_td && count($errors) === 0 && (bcsub((string) $depn_td, $asset->td_depn)) != 0)
		{

			$gltrans = GLTransaction::makeFromAsset($asset, bcsub((string) $depn_td, $asset->td_depn), 'depreciation', $errors);
