...
function smarty_function_datetime($params, &$smarty) {
	...
	$data['label']['value'] = prettify(($params['attribute'] ?? ''));
...
#EOT;
	...
	$data['label']['value'] = ($params['label'] ?? $data['label']['value']);
	...
	if (!empty($value)) 
	{
		...
		if (strptime($value, $format) !== FALSE)
		{
			...
			$minute_value	= array_pop(explode(':', (string) array_pop(explode(' ', (string) $value))));
		}
		else
		{
			list($date_value, $rest) = explode(' ', (string) $value,2);
