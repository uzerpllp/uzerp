...
class http_class
{
 ...
 private function _StreamRequest ($arguments)
 {
  ...
  if (!$this->connected)
  {
   return self::_HttpError (_("not connected"), E_USER_WARNING);
  }
  ...
  foreach ($this->arguments["BodyStream"] as $argument)
  {
   ...
   elseif ($type == "File")
   {
    ...
    else
    {
     $length = 0;
     return
      self::_HttpError (sprintf (_("%s: file is not readable"), $value),
        E_USER_WARNING);
    }
   }
   else
  ...
  {
   return self::_HttpError (_("Error while puts POST operation"),
     E_USER_WARNING);
  }
  foreach ($this->arguments["Headers"] as $header => $value)
  {
   ...
   if (!$this->_streamString ($string))
   {
    return self::_HttpError (_("Error while puts HTTP headers"),
      E_USER_WARNING);
   }
  }
  $string = "\r\n";
  if (!$this->_streamString ($string))
  {
   return self::_HttpError (_("Error while ends HTTP headers"),
     E_USER_WARNING);
  }
  foreach ($this->arguments["BodyStream"] as $argument)
  {
    ...
    if (is_readable ($value))
    {
     ...
     while (!feof ($file))
     {
      if (gettype ($block = @fread ($file, $this->window_size)) !=
        "string")
      {
       return self::_HttpError (_("cannot read file to upload"),
         E_USER_WARNING);
      }
