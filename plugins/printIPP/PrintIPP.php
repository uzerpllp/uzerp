...
// OPERATIONS
    ...
    public function printJob(){
        ...
        if (isset($this->serveroutput) && isset($this->serveroutput->status)) {
            ...
            else
            {
                $this->jobs = array_merge($this->jobs,array(""));
                $this->jobs_uri = array_merge($this->jobs_uri,array(""));
                self::_errorLog(sprintf("printing job: %s",$this->last_job) .$this->serveroutput->status,1);
    ...
    public function validateJob () {
        ...
        if (!isset($this->setup->jobname))
            if (is_readable($this->data))
                self::setJobName(basename((string) $this->data),true);
            else
                self::setJobName();
...
// RESPONSE PARSING
    ...
    protected function _interpretAttribute($attribute_name,$type,$value) {
        ...
        switch ($type) {
            ...
            case 'resolution':
                $unit = $value[8];
                $value = self::_interpretRangeOfInteger(substr((string) $value,0,8));
                switch($unit) {
                    case chr(0x03):
                        $unit = "dpi";
                        break;
    ...
    protected function _interpretRangeOfInteger($value) {
        ...
        $halfsize = strlen((string) $value) / 2;

        $integer1 = self::_interpretInteger(substr((string) $value,0,$halfsize));
        $integer2 = self::_interpretInteger(substr((string) $value,$halfsize,$halfsize));
    ...
    protected function _interpretDateTime($date) {
        $year = self::_interpretInteger(substr((string) $date,0,2));
        $month =  self::_interpretInteger(substr((string) $date,2,1));
        $day =  self::_interpretInteger(substr((string) $date,3,1));
        $hour =  self::_interpretInteger(substr((string) $date,4,1));
        $minute =  self::_interpretInteger(substr((string) $date,5,1));
        $second =  self::_interpretInteger(substr((string) $date,6,1));
        $direction = substr((string) $date,8,1);
        $hours_from_utc = self::_interpretInteger(substr((string) $date,9,1));
        $minutes_from_utc = self::_interpretInteger(substr((string) $date,10,1));
